...
public:
  ...
  void AddTruthMET    (const std::string& detailStr = "", const std::string& truthMetName = "TruthMET");
  ...
  void FillTruthMET( const xAOD::MissingETContainer* truthMet, const std::string& truthMetName = ""/*"TruthMET"*/);
  ...
  void ClearTruthMET	  (const std::string& truthMetName = "TruthMET");
  ...
  //Added by JD to the mic MC
  virtual void AddTruthMETUser(const std::string& truthMetName = "", const std::string& detailStr = "TruthMET")
    {
      if(m_debug) Info("AddTruthMETUser", "Empty function called from HelpTreeBase %s %s", truthMetName.c_str(), detailStr.c_str());
      return;
    };
  ...
  virtual void ClearTruthMETUser    (const std::string& /*truthMetName = "TruthMET"*/) { return;};
  ...
  virtual void FillTruthMETUser( const xAOD::MissingETContainer*, const std::string& /*truthMetName = "TruthMET"*/ ) { return; };
...
protected:
  ...
  // TruthMET
  //
  std::map<std::string, xAH::TruthMET*> m_TruthMet;
